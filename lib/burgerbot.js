...
class BurgerBot extends Bot {
    constructor(settings) {
        ...
        // super-constructor call to parent class' constructor
...
BurgerBot.prototype._replyWithLatestSpecial = function (originalMessage) {
    var self = this;
    var url = 'http://www.honestburgers.co.uk/news';
...
BurgerBot.prototype._getBurgerDetails = function (url, errorMsg, callback) {
    request(url, function(error, response, html){
        if(!error){
            ...
            $('#main > .news > div:first-child > article').each(function(i, el) {
                var data = $(this);
                var contents = data.text();
                if (contents.toLowerCase().indexOf('now serving') !== -1) {
                    title = data.find('.news_span').text();
                    description = data.find('p').clone().children().remove().end().text().trim();
